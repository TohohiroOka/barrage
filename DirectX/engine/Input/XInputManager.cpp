...
bool XInputManager::TriggerButton(const int _button)
{
	bool check = false;
	if (_button == PAD_BUTTON::PAD_A &&
		controllers.state.Gamepad.wButtons & XINPUT_GAMEPAD_A &&
...
bool XInputManager::GetTriggerButton(int& button)
{
	for (int i = 0; i < 256; i++) {
		if (TriggerButton(i)) {
			button = i;
